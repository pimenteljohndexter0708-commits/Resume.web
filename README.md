# Personal Portfolio Website

## Student Information
Name: John Dexter Pimentel
Course: BSIT
Section: 3A

## Project Description

A single-page personal portfolio website built with plain HTML/CSS/JS. It features a bento-style layout with an identity block, an interactive photo gallery, and quick-reference info sections such as About Me, Educational Background, and Contact Information.

## Features

- Responsive layout (desktop, tablet, and mobile breakpoints)
- Bento-grid sections: Special Talent, Example, Motto, Dream
- Photo gallery with click-to-open lightbox
- About Me, Educational Background, and Contact Information section
- No frameworks or build tools — just HTML, CSS, and vanilla JS

## Tech Used

- HTML5
- CSS3 (Grid & Flexbox)
- Vanilla JavaScript
- Google Fonts (Space Grotesk, IBM Plex Mono, Fraunces, Work Sans)

## Website Link

- https://pimenteljohndexter0708-commits.github.io/Resume.web/

## Github Repository

- https://github.com/pimenteljohndexter0708-commits/Resume.web
